...
$liquidWindow.on("pageshow load", ev => {
  if (typeof jQuery.fn.liquidPreloader === "function") {
    jQuery(".lqd-preloader-wrap").liquidPreloader();
  } else {
    console.warn("Plugin liquidPreloader não está disponível.");
  }
});
(function ($) {
  "use strict";
  ...
  class Plugin {
    ...
    init() {
      ...
      this.$submenuParents.each(async (i, subParent) => {
        ...
        const submenu = subParent.querySelector(
          ...
          new IntersectionObserver(([entry], observer) => {
            fastdom.measure(() => {
              ...
              if (x < 0) {
                ...
                x =
                  windowWidth - width - entry.target.parentElement.offsetWidth;
              }
              if (y < 0) {
                y = y + window.scrollY;
              }
...
(function ($) {
  ...
  class Plugin {
    ...
    initMenuItemsCursor() {
      ...
      const magneticMenuItems = document.querySelectorAll(
        ".lqd-magnetic-items"
      );
    ...
    firstSectionIO() {
      ...
      new IntersectionObserver(
        ([entry]) => {
          ...
          if (firstSectionIsSticky) {
            activate =
              rootBounds && rootBounds.bottom >= boundingClientRect.top;
          }
          if (firstSectionIsOnlyChild) {
            activate =
              rootBounds &&
...
(function ($) {
  ...
  class Plugin {
    ...
    onEndTransFn(direction) {
      ...
      if (!direction || direction === "next") {
        this.draggie.element.style.left = this.draggie.element.style.top =
          "0px";
  ...
  class Plugin {
    ...
    initSelect() {
      ...
      if (!$selectElement.closest(".variations").length) {
        ...
        $selectElement.wrap(
          '<span class="lqd-select-wrap pos-rel relative" />'
        );
  ...
  class Plugin {
    ...
    asSectionBg() {
      if (this.options.asBG) {
        ...
        const elementContainer = this.$element.parent(
...
(function ($) {
  ...
  class Plugin {
    ...
    initCircleProgressbar(value) {
      const circleContainer = $(this.element).find(
        ".ld-prgbr-circle-container"
      );
...
(function ($) {
  ...
  class Plugin {
    ...
    animateElements() {
      ...
      const $currentExtrasChildElements = $currentExtras
        .children()
        .not("style");
      gsap.fromTo(
        $prevExtras[0],
...
(function ($) {
  ...
  class Plugin {
    ...
    createSnickersBar($btn, data) {
      ...
      const snickersClone =
        this.DOM.$snickersBarTemp[0].content.cloneNode(true);
...
(function ($) {
  ...
  const localscrollGlobalOffset =
    bodyLSOffset != null && bodyLSOffset !== ""
      ? parseInt(bodyLSOffset, 10)
      : 0;
  ...
  class Plugin {
    ...
    getTargetsArray(itemIndex, item) {
      ...
      // Proteção contra hash inválido (ex: "#/rota", "#!anchor", etc)
      if (
        !hash ||
        hash === "" ||
        hash.includes("/") ||
      ...
      try {
        ...
        console.warn("Hash inválido ignorado:", hash);
        return (this.targetsArray[itemIndex] = null);
      }

      // Tudo certo: armazena o elemento alvo
  ...
  // 🔐 Proteção contra erro de collapse()
  const selectors = [
    ".navbar-fullscreen",
    ".navbar-collapse",
    ".ld-module-dropdown",
  ...
  selectors.forEach(sel => {
    const $el = this.$element.closest(sel);
    if ($el.length && typeof $el.collapse === "function") {
      $el.collapse("hide");
    }
  });
}
  ...
  class Plugin {
    ...
    _addImages() {
      $.each(this.imageArray, (i, { src, alt }) => {
        ...
        const $slideshowItemInner = $(
          '<div class="ld-slideshow-item-inner" />'
        );
        $figure.append($img);
  ...
  class Plugin {
    ...
    build() {
      ...
      if ($customAnimationChild.length) {
        const data = $customAnimationChild.data(
          "plugin_liquidCustomAnimations"
        );
        data && promises.push(data.animationsDonePromise);
      }
